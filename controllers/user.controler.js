...
const UserConter = {
    GET: async (req, res, next) => {
        try {
            ...
            let user = { role: "admin" }; //await fetch(GET, id);
            if (user.role == "admin") {
                ...
                if (userId) {
                    ...
                    if (users.length)
                        ...
                        throw new Error(
                            "Users not found! Foydalanuvchilar topilmadi!"
                        );
                }
    ...
    PUT: async (req, res, next) => {
        try {
            if (req.user.id) {
                ...
                throw new Error(
                    "You don't have permission! Sizga ruxsat berilmagan!"
                );
        } catch (err) {
            res.status(400).json({
                status: 404,
