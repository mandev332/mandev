...
const auth = {
    TOKEN: async (req, res, next) => {
        try {
            ...
            if (jwt.VERIFY(token) instanceof Error)
                throw new Error(
                    "You are not registered! Siz ro'yxatdan o'tmagansiz!"
                );
    ...
    LOGIN: async (req, res, next) => {
        try {
            ...
            res.send({
                ...
                message:
                    "Welcome - Save a token! Xush kelibsiz - Tokenni saqlang!",
            });
        } catch (error) {
            res.status(400).json({
                status: 401,
    ...
    CHECK: async (req, res, next) => {
        try {
            ...
            const mailOptions = {
                ...
                to: "nodirbek332qobilov@gmail.com",
                subject: "GETTER UZ",
    ...
    UPLOAD: async (req, res, next) => {
        try {
            ...
            avatar =
                "/avatarka/users/" + (contact || user.contact) + "." + type;
    ...
    REGISTER: async (req, res, next) => {
        try {
            ...
            let response = await fetch(
                ...
                sha256(password),
                avatar ||
                    (gender == "male" ? "/users/boy.jpg" : "/users/girl.jpg"),
                profession,
                gender
            );
            ...
            res.send({
                ...
                message:
                    "User added in users! Foydalanuvchi ro'yxatga qo'shildi!",
            });
        } catch (error) {
            res.status(400).json({
                status: 401,
